...
//       {/* Business List */}
//       <div>
//         <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200 mb-6">My Portfolios</h2>
...
export default function Dashboard() {
  ...
  return (
    <div className="p-4 md:p-8 space-y-8 max-w-7xl mx-auto animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {businesses.map((business) => (
                    <div key={business.id} className="relative group bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 hover:shadow-lg transition-all">
                        ...
                        <div className="flex justify-between items-start mb-4">
                            <Link href={`/dashboard/business/${business.id}`} className="flex-1 hover:text-[hsl(var(--primary))] transition-colors pr-2">
                                <h3 className="font-bold text-lg text-slate-900 dark:text-white truncate">{business.name}</h3>
                            </Link>
                                ...
                                {activeMenu === business.id && (
                                    ...
                                    <div className="absolute right-0 top-8 z-20 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl w-40 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                        ...
                                        <button 
                                            ...
                                            className="w-full text-left px-4 py-3 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2 border-b border-slate-100 dark:border-slate-700/50"
                                        >
                                            <Pencil className="w-4 h-4" /> Edit Name
                                        </button>
                                        ...
                                        <button 
                                            ...
                                            className="w-full text-left px-4 py-3 text-sm text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/20 flex items-center gap-2"
                                        >
      ...
      {editingBiz && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
            <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-2xl shadow-2xl p-6 border border-slate-200 dark:border-slate-800">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-bold dark:text-white">Rename Portfolio</h3>
                    <button onClick={() => setEditingBiz(null)} disabled={isRenaming}><X className="w-5 h-5 text-slate-400" /></button>
                </div>
                <input 
                    value={newName}
                    onChange={(e) => setNewName(e.target.value)}
                    ...
                    autoFocus
                />
                <Button onClick={handleRename} className="w-full" isLoading={isRenaming}>Save Changes</Button>
            </div>
        </div>
      )}
      ...
      {deletingBiz && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
            <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-2xl shadow-2xl p-6 border border-slate-200 dark:border-slate-800 text-center">
                <div className="mx-auto w-12 h-12 bg-rose-100 dark:bg-rose-900/20 text-rose-600 rounded-full flex items-center justify-center mb-4">
                ...
                <div className="flex gap-3">
                    ...
                    <Button variant="danger" className="flex-1" onClick={handleDelete} isLoading={isDeleting}>Delete</Button>
                </div>
            </div>
        </div>
      )}
